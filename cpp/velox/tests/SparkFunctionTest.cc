...
TEST_F(SparkFunctionTest, locateTwoArgs) {
  const auto locate = [&](const std::optional<std::string>& substr, const std::optional<std::string>& str) {
    return evaluateOnce<int32_t>("locate(c0, c1)", substr, str);
  };

  // Basic cases
  EXPECT_EQ(locate("aa", "aaads"), 1);
  EXPECT_EQ(locate("aa", "baaads"), 2);
  EXPECT_EQ(locate("de", "aaads"), 0);
  EXPECT_EQ(locate("abc", "abcdddabcabc"), 1);
  ...
  EXPECT_EQ(locate("xyz", "aaads"), 0);

  // NULL handling
  EXPECT_EQ(locate("aa", std::nullopt), std::nullopt);
  EXPECT_EQ(locate(std::nullopt, "aaads"), std::nullopt);
  EXPECT_EQ(locate(std::nullopt, std::nullopt), std::nullopt);

  // Unicode support
  EXPECT_EQ(locate("\u7231", "\u4FE1\u5FF5,\u7231,\u5E0C\u671B"), 4);
  EXPECT_EQ(locate("\u4FE1", "\u4FE1\u5FF5,\u4FE1\u7231,\u4FE1\u5E0C\u671B"), 1);
}

TEST_F(SparkFunctionTest, locateThreeArgs) {
  ...
  // Basic cases with start position
  ...
  // Start position less than 1 returns 0
  ...
  // Start position less than 1 with empty substring returns 0
